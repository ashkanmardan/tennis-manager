...
- **Asset provenance.** Vazirmatn is SIL OFL 1.1; its license notice is now included. On 2026-09-20, the maintainer confirmed that they designed the checked-in launcher icons. All ten launcher PNGs matched the clean original local checkout byte-for-byte. The checked-in artwork differs from the output of `make_icons.ps1`; `fix_icons.ps1` records a copy from another local project path that is no longer available. The maintainer's authorship confirmation resolves the icon-rights question for this audit, while the script history remains worth documenting. The MIT license covers the original project code, documentation, and icon artwork; it does not override third-party licenses.
