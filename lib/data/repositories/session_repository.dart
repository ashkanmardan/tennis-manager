...
class SessionRepository {
  ...
  Future<Session?> getNextUpcoming() async {
    ...
    final rows = await db.query('sessions',
        where: "status = 'upcoming' AND scheduled_date >= ? AND scheduled_date != '9999-01-01'",
        whereArgs: [today],
        orderBy: 'scheduled_date ASC',
        limit: 1);
  ...
  /// جلسه گذشته‌ای که هنوز وضعیتش ثبت نشده (upcoming است اما تاریخش گذشته)
  Future<Session?> getUnresolvedPastSession() async {
    final db = await _db;
    final today = DateTime.now().toIso8601String().substring(0, 10);
    final rows = await db.query('sessions',
        where: "status = 'upcoming' AND scheduled_date < ? AND scheduled_date != '9999-01-01'",
